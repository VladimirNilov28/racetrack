...
describe.skip("scenario: manual finish + mode lock + no laps after end", () => {
    beforeEach(() => {
        vi.useFakeTimers();
    ...
    it("finish locks mode, laps are blocked after finish/end, endSession resets system", () => {
        ...
        expect(
            state.sessions.current.drivers.find((d) => d.car === 7).laps,
        ).toBe(1);
        ...
        expect(() => recordLap(state, 7)).toThrow(
            /finish|ended|not started|no active/i,
        );

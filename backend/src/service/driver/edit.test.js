...
describe("driver/edit", () => {
  ...
  it("throws if sessionId is missing", () => {
    ...
    expect(() => editDriver(state, null, 1, { name: "X" })).toThrow(/session.*id/i);
    expect(() => editDriver(state, "", 1, { name: "X" })).toThrow(/session.*id/i);
  });
  ...
  it("throws if carNumber is missing", () => {
    ...
    expect(() => editDriver(state, "S1", null, { name: "X" })).toThrow(/car/i);
    expect(() => editDriver(state, "S1", undefined, { name: "X" })).toThrow(/car/i);
  });
  ...
  it("throws if session is not found in current or upcoming", () => {
    ...
    expect(() => editDriver(state, "S999", 1, { name: "X" })).toThrow(/session.*not found/i);
  });
  ...
  it("does not allow editing drivers in lastResult", () => {
    ...
    expect(() => editDriver(state, "R1", 1, { name: "X" })).toThrow(/lastresult|not allowed/i);
  });
  ...
  it("throws if driver is not found by carNumber in the target session", () => {
    ...
    expect(() => editDriver(state, "S1", 99, { name: "X" })).toThrow(/driver.*not found/i);
  });

  it("edits driver name in an upcoming session (keeps timing stats untouched)", () => {
    const base = createInitialState();
    const d1 = makeDriver({ name: "Alice", car: 7, laps: 3, lastLapAt: 1000, fastestLap: 250 });
    ...
    const updatedSession = result.sessions.upcoming.find(s => s.id === "S1");
  ...
  it("edits driver car in current session (car must stay unique)", () => {
    ...
    expect(result.sessions.current.drivers.map(d => d.car).sort((a, b) => a - b))
      .toEqual([12, 99]);
  });
  ...
  it("throws if new car already exists in the same session", () => {
    ...
    expect(() => editDriver(state, "CUR", 7, { car: 12 })).toThrow(/car.*exists|car.*unique/i);
  });
  ...
  it("does not mutate original state (immutability smoke test)", () => {
    ...
    const state = {
      ...
      sessions: {
        ...base.sessions,
        upcoming: [makeSession("S1", [makeDriver({ name: "Alice", car: 7 })])],
      },
    };
    ...
    expect(afterSessionRef.drivers[0]).not.toBe(beforeSessionRef.drivers[0]);
  });
});

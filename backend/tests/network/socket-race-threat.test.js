 ...
 */
async function waitForEventDriven(socket, eventName, predicate, { timeoutMs = 2000, stepMs = 20 } = {}) {
    ...
    while (Date.now() - startedAt <= timeoutMs) {
        const res = await Promise.race([
            done.then((v) => ({ ok: true, v })).catch((e) => ({ ok: false, e })),
            (async () => {
    ...
    throw new Error(`Timeout waiting for ${eventName}. Last payload: ${JSON.stringify(lastPayload)}`);
}

async function onceEventDriven(socket, eventName, { timeoutMs = 2000, stepMs = 20 } = {}) {
...
async function createTestServer({ intervalMs = 20 } = {}) {
    ...
    const [{ EVENTS }, { keyAuthentication }, { socketConnect, socketDisconnect }, { startTicker }, keys] =
        await Promise.all([
...
describe("network mini: sockets", () => {
    ...
    it("connects and receives initial STATE_UPDATE snapshot", async () => {
        ...
        const st0 = await onceEventDriven(observer, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });
    ...
    it("rejects connection or commands when role has wrong key", async () => {
        ...
        // auth.js делает delayMs: 500 -> нужно ПРОКРУТИТЬ fake-time
        const err = await waitForEventDriven(
            bad,
            "connect_error",
            (e) => Boolean(e),
    ...
    it("front-desk can add session and observer sees it in STATE_UPDATE", async () => {
        ...
        await onceEventDriven(receptionist, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });
        await onceEventDriven(observer, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });
        ...
        const st = await waitForEventDriven(
            ...
            (s) => Array.isArray(s?.sessions?.upcoming) && s.sessions.upcoming.some((x) => x.id === "S1"),
    ...
    it("race-control can start race: current set + timer running", async () => {
        ...
        await onceEventDriven(receptionist, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });
        await onceEventDriven(control, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });
        await onceEventDriven(observer, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });

        expect(
            (await emitCmd(receptionist, srv.CMD.SESSION_ADD, { session: { id: "S1", drivers: [] } })).ok
        ).toBe(true);

        expect(
            (await emitCmd(receptionist, srv.CMD.DRIVER_ADD, { sessionId: "S1", driver: { name: "A", car: 7 } })).ok
        ).toBe(true);

        const startAck = await emitCmd(control, srv.CMD.RACE_START, { durationSec: 1 });
        expect(startAck.ok).toBe(true);

        const st = await waitForEventDriven(
            observer,
            srv.EVT.STATE_UPDATE,
            (s) => s?.sessions?.current?.id === "S1" && s?.timer?.status === "running",
            { timeoutMs: 1500, stepMs: 20 }
        );
    ...
    it("race finish flow: after timer ends -> lastResult set; NEXT start is optional", async () => {
        ...
        // Collect lastResult history to prove S1 happened even if we later reach S2
        ...
        expect((await emitCmd(receptionist, srv.CMD.SESSION_ADD, { session: { id: "S1", drivers: [] } })).ok).toBe(true);
        expect((await emitCmd(receptionist, srv.CMD.SESSION_ADD, { session: { id: "S2", drivers: [] } })).ok).toBe(true);

        expect((await emitCmd(receptionist, srv.CMD.DRIVER_ADD, { sessionId: "S1", driver: { name: "A", car: 7 } })).ok).toBe(true);
        expect((await emitCmd(receptionist, srv.CMD.DRIVER_ADD, { sessionId: "S2", driver: { name: "B", car: 21 } })).ok).toBe(true);

        expect((await emitCmd(control, srv.CMD.RACE_START, { durationSec: 1 })).ok).toBe(true);
        ...
        // Jump beyond end of S1 and let ticker run
        vi.setSystemTime(endsAt + 5);
        await vi.advanceTimersByTimeAsync(60);

        // Wait until system reaches a stable "post-finish" state:
        // either ended fully (idle/current null) OR auto-started S2 (running/current S2)
        const post = await waitForEventDriven(
            observer,
            srv.EVT.STATE_UPDATE,
            (s) => {
                const ended = s?.timer?.status === "idle" && s?.sessions?.current === null;
                const autoNext = s?.timer?.status === "running" && s?.sessions?.current?.id === "S2";
                return ended || autoNext;
            },
            { timeoutMs: 2000, stepMs: 20 }
        );

        // Key assertion: S1 must have appeared as lastResult at some point
        expect(seenLastResults.includes("S1")).toBe(true);

        // Additional sanity: if we ended, lastResult could be S1 or even S2 (if time advanced a lot)

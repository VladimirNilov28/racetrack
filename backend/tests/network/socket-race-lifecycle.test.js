...
describe.skip("network scenario: full socket lifecycle (2 races back-to-back)", () => {
    const opened = [];
    ...
    it("runs 2 races via real sockets: sessions+drivers -> start -> laps -> timer ends -> next -> end", async () => {
        ...
        const connErrors = [];
        receptionist.on("connect_error", (e) =>
            connErrors.push(["receptionist", e]),
        );
        raceControl.on("connect_error", (e) =>
            connErrors.push(["raceControl", e]),
        );
        observer.on("connect_error", (e) => connErrors.push(["observer", e]));

        await receptionistState.wait((st) => !!st, {
            timeoutMs: 6000,
            debugLabel: "initial receptionist",
        });
        await raceControlState.wait((st) => !!st, {
            timeoutMs: 6000,
            debugLabel: "initial raceControl",
        });
        await observerState.wait((st) => !!st, {
            timeoutMs: 6000,
            debugLabel: "initial observer",
        });
        ...
        res = await emitCmd(receptionist, CMD.SESSION_ADD, {
        ...
        await observerState.wait(
            (st) =>
                st?.sessions?.current?.id === "S1" &&
                st.sessions.current.drivers?.some(
                    (d) => d.car === 7 && d.laps === 1,
                ),
        ...
        s1 = await observerState.wait(
            (st) => {
                ...
                const d7 = st.sessions.current.drivers?.find(
                    (d) => d.car === 7,
                );
                return d7?.laps === 2;
            },
        ...
        await observerState.wait(
            (st) => {
                if (st?.sessions?.current?.id !== "S1") return false;
                const d12 = st.sessions.current.drivers?.find(
                    (d) => d.car === 12,
                );
                return d12?.laps === 1;
            },
        ...
        const isS2Running =
            s2Start?.sessions?.current?.id === "S2" &&
            s2Start?.timer?.status === "running";

        if (!isS2Running) {
            res = await emitCmd(raceControl, CMD.RACE_START, {
                durationSec: 1,
            });
            expectOk(res);
        ...
        const s2 = await observerState.wait(
            (st) => {
                ...
                const d21 = st.sessions.current.drivers?.find(
                    (d) => d.car === 21,
                );
                return d21?.laps === 2;
            },

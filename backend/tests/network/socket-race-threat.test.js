 ...
 */
async function connectAndWaitInitial(socket, stateEventName, { timeoutMs = 1000, stepMs = 20 } = {}) {
...
describe("network mini: sockets", () => {
  ...
  it("connects and receives initial STATE_UPDATE snapshot", async () => {
    ...
    const st0 = await connectAndWaitInitial(observer, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });
  ...
  it("front-desk can add session and observer sees it in STATE_UPDATE", async () => {
    ...
    await connectAndWaitInitial(receptionist, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });
    await connectAndWaitInitial(observer, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });
  ...
  it("race-control can start race: current set + timer running", async () => {
    ...
    await connectAndWaitInitial(receptionist, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });
    await connectAndWaitInitial(control, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });
    await connectAndWaitInitial(observer, srv.EVT.STATE_UPDATE, { timeoutMs: 1000 });

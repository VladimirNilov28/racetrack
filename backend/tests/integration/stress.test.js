...
describe("MEGA scenario: lifecycle + guardrails + invariants", () => {
    ...
    it.skip("runs 2 races back-to-back and enforces key invariants", () => {
        ...
        expect(() => editDriver(state, "S1", 7, { car: 12 })).toThrow(
            /car|exists|duplicate|unique/i,
        );
        ...
        expect(s1Live.drivers.find((d) => d.car === 7).fastestLap).not.toBe(
            null,
        );
        ...
        expect(() => recordLap(state, 21)).toThrow(
            /finish|finished|ended|running|started/i,
        );

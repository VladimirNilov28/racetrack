...
describe("driver/add", () => {
  ...
  it("throws if sessionId is missing", () => {
    ...
    expect(() => addDriver(state, null, { name: "A", car: 1 })).toThrow(/session.*id/i);
    expect(() => addDriver(state, "", { name: "A", car: 1 })).toThrow(/session.*id/i);
  });
  ...
  it("throws if session is not found in current or upcoming", () => {
    ...
    expect(() => addDriver(state, "S999", { name: "Alice", car: 1 })).toThrow(/session.*not found/i);
  });
  ...
  it("adds a driver to an upcoming session by sessionId (with defaults)", () => {
    ...
    expect(updated.drivers).toEqual([
      { name: "Alice", car: 7, laps: 0, lastLapAt: null, fastestLap: null },
    ]);
  });
  ...
  it("adds a driver to the current session by sessionId (with defaults)", () => {
    ...
    expect(result.sessions.current.drivers).toEqual([
      { name: "Bob", car: 12, laps: 0, lastLapAt: null, fastestLap: null },
    ]);
  });
  ...
  it("throws if car number already exists in the target session", () => {
    ...
    expect(() => addDriver(state, "S1", { name: "Eve", car: 7 })).toThrow(/car/i);
  });
  ...
  it("does not allow adding drivers to lastResult session", () => {
    ...
    expect(() => addDriver(state, "R1", { name: "Alice", car: 7 })).toThrow(/lastresult|finished|not allowed/i);
  });
